...
impl SinBackward {
    fn backward_metal(&self, grad_output: &Tensor) -> TensorResult<Tensor> {
        ...
        let pipeline = library
            .get_function("sin_backward_f16", None)
            .map_err(|e| {
                TensorError::MetalError(format!("Failed to get kernel sin_backward_f16: {:?}", e))
            })?;
...
impl CosBackward {
    fn backward_metal(&self, grad_output: &Tensor) -> TensorResult<Tensor> {
        ...
        let library = library_ref
            ...
            .ok_or_else(|| TensorError::MetalError("Library not loaded".to_string()))?;
        let pipeline = library
            .get_function("cos_backward_f16", None)
            .map_err(|e| {
                TensorError::MetalError(format!("Failed to get kernel cos_backward_f16: {:?}", e))
            })?;
