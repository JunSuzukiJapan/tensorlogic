...
impl SigmoidBackward {
    fn backward_metal(&self, grad_output: &Tensor) -> TensorResult<Tensor> {
        ...
        let pipeline = library
            .get_function("sigmoid_backward_f16", None)
            .map_err(|e| {
                TensorError::MetalError(format!(
                    "Failed to get kernel sigmoid_backward_f16: {:?}",
                    e
                ))
            })?;
...
impl TanhBackward {
    fn backward_metal(&self, grad_output: &Tensor) -> TensorResult<Tensor> {
        ...
        let library = library_ref
            ...
            .ok_or_else(|| TensorError::MetalError("Library not loaded".to_string()))?;
        let pipeline = library
            .get_function("tanh_backward_f16", None)
            .map_err(|e| {
                TensorError::MetalError(format!("Failed to get kernel tanh_backward_f16: {:?}", e))
            })?;
