// Optimize residual scaling coefficient for FULL SEQUENCE mode
// Uses real model weights with 29-token sequence

use tensorlogic::device::MetalDevice;
use tensorlogic::tensor::Tensor;
use tensorlogic::model::formats::gguf::GGUFLoader;
...
fn test_coefficient(
    ...
    // Create input [seq_len, hidden_dim] with realistic values
    let input_data: Vec<f32> = (0..(seq_len * hidden_dim))
        .map(|i| {
            let pos = i / hidden_dim;
            let dim = i % hidden_dim;
            ((pos as f32 * 0.1 + dim as f32 * 0.01) % 2.0 - 1.0) * 0.1
        })
        .collect();
    let input_data_f16: Vec<half::f16> = input_data.iter()
        .map(|&x| half::f16::from_f32(x))
        .collect();
    let input = Tensor::from_vec_metal(device, input_data_f16.clone(), vec![seq_len, hidden_dim]).unwrap();

    let mut x = input.clone();
    let (_, _, _, initial_std) = tensor_stats(&x);

    // Simulate 22 layers with residual scaling
    for _ in 0..22 {
        ...
        let proj_scaled_data: Vec<half::f16> = proj_data.iter()
            ...
            .collect();
        let proj_scaled = Tensor::from_vec_metal(device, proj_scaled_data, vec![seq_len, hidden_dim]).unwrap();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Full Sequence Mode: Residual Scaling Optimization ===\n");
    println!("Sequence length: 29 tokens (realistic chat input)");
    println!("Target: Find coefficient where std growth = 1.3x\n");
    ...
    let model = GGUFLoader::load(&model_path, &device)?;

    // Use layer 0 weights as representative
    let norm_weight = model.get_tensor("blk.0.attn_norm.weight")
        .ok_or("Failed to load norm weight")?;
    let weight = model.get_tensor("blk.0.attn_q.weight")
        .ok_or("Failed to load Q weight")?;

    println!("✓ Model weights loaded\n");

    let target_growth = 1.3;

    // Phase 1: Coarse search (0.0 to 0.5, step 0.01)
    println!("=== PHASE 1: Coarse Search (0.00 to 0.50, step 0.01) ===\n");
    ...
    for i in 0..=50 {
        ...
        let (final_std, std_growth) = test_coefficient(&device, coeff, &norm_weight, &weight, seq_len, hidden_dim);
        ...
        // Print every 5th result in Phase 1
        if i % 5 == 0 {
    ...
    for (coeff, _growth, rating) in &phase1_results {
    ...
    println!("Mode: FULL SEQUENCE (29 tokens, 22 layers)");
    println!("Optimal Coefficient: {:.3}", optimal_coeff);
    println!("Std Growth: {:.2}x", optimal_growth);
    println!("Best Rating: {:.1}", optimal_rating);
    println!();
    println!("Comparison:");
    println!("  - Incremental mode (1 token): 0.151");
    println!("  - Full sequence mode (29 tokens): {:.3}", optimal_coeff);
    println!();
    println!("Recommendation:");
    println!("  Use residual_scale = {:.3} for full sequence processing", optimal_coeff);
