...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let input = Tensor::from_vec_metal(&device, input_data_f16.clone(), vec![seq_len, hidden_dim])?;
    ...
    let norm_weight_data_f16: Vec<half::f16> = norm_weight_data.iter()
        .map(|&x| half::f16::from_f32(x))
        .collect();
    let norm_weight = Tensor::from_vec_metal(&device, norm_weight_data_f16, vec![hidden_dim])?;
